...
from common_utils.regex_functions import replace_re_special
import gc 
import re 
import warnings 
import traceback 
...
def merge_case_insensitive(df_1,df_2,how, on):
    ...
    for i, o in enumerate(on) :
        ...
        df_1[temp_name] = df_1[o].apply(lambda x: x.lower().strip() if type(x) == str else x )
        df_2[temp_name] = df_2[o].apply(lambda x: x.lower().strip() if type(x) == str else x )

        # rename_dict.update({o:temp_name})
    ...
    for column, split_standard_list in split_words_dict.items():
        ...
        try:
            if split_standard_list:
                #找到所有能匹配上的 进行拆分
                df[column] = df[column]\
                    .apply(lambda x: [ x if type(x) == str and x.strip() != '' else x for x in re.split(split_pat,x.strip())])
                df[column] = df[column]\
                    .apply(lambda x: [ y for y in x if (type(y)==str and y !='' and y != split_symbol) and y == y ])
            elif split_symbol != '':
                df[column] = df[column]\
...
def group_basic_agg(df,group_column,agg_func):
    """
    根据分类统计基本的类别数量
    :param df : input df 
    ...
    if type(agg_func) == dict:
        agg_func_column = list(agg_func.keys())
    else:
    ...
    df_copy_agg =  df_copy.groupby(group_column).agg(agg_func).sort_values(by=agg_func_column,ascending=False).reset_index()
...
def calc_total(df,total_column=0):
    ...
    df_copy[total_column][last_index] = '总计'
...
def get_outer_join_diff(df_list,on=None,left_right=None):
    ...
    outer_join_diff_df = outer_join_diff_df.rename({'table_source_1':'来源'},axis=1)
    #删除已经填充过去的列
